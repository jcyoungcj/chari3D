...
static double result[height][width];									 //保存每个点的三维高度
//static Mat out2(height, width, CV_64FC1, Scalar(0));
...
void dfs(Mat guhua, int x, int y, int w, int l, int line_panduan,Mat image_pixel){
	...
	else if (x - 1 >= 0 && x<l && y - 1 >= 0 && y <w && guhua.at<double>(x - 1, y - 1)>0){
		image_pixel.at<double>(line_panduan, y - 1) = guhua.at<double>(x - 1, y - 1);
		return dfs(guhua, x - 1, y - 1, w, l, line_panduan, image_pixel);
	}
	else if (x >= 0 && x + 2<l && y - 1 >= 0 && y <w && guhua.at<double>(x + 2, y - 1)>0){
		image_pixel.at<double>(line_panduan, y - 1) = guhua.at<double>(x + 2, y - 1);
		return dfs(guhua, x +2, y - 1, w, l, line_panduan, image_pixel);
	}
	else if (x >= 0 && x + 1<l && y - 1 >= 0 && y<w && guhua.at<double>(x + 1, y - 1)>0){
		image_pixel.at<double>(line_panduan, y - 1) = guhua.at<double>(x +1, y - 1);
		return dfs(guhua, x + 1, y - 1, w, l, line_panduan, image_pixel);
	}
}
...
void dfs2(Mat guhua, int x, int y, int w, int l, int line_panduan, Mat image_pixel){
	...
	else if (x - 2 >= 0 && x<l && y >= 0 && y + 1<w && guhua.at<double>(x - 2, y + 1)>0){
		image_pixel.at<double>(line_panduan, y + 1) = guhua.at<double>(x - 2, y + 1);
		return dfs2(guhua, x - 2, y + 1, w, l, line_panduan, image_pixel);
	}
	else if (x - 1 >= 0 && x<l && y >= 0 && y + 1<w && guhua.at<double>(x - 1, y + 1)>0){
		image_pixel.at<double>(line_panduan, y + 1) = guhua.at<double>(x - 1, y + 1);
		return dfs2(guhua, x - 1, y + 1, w, l, line_panduan, image_pixel);
	}
	else if (x >= 0 && x + 2<l && y >= 0 && y + 1<w && guhua.at<double>(x + 2, y + 1)>0){
		image_pixel.at<double>(line_panduan, y + 1) = guhua.at<double>(x + 2, y + 1);
		return dfs2(guhua, x + 2, y + 1, w, l, line_panduan, image_pixel);
	}
	else if (x >= 0 && x + 1 < l && y >= 0 && y + 1 < w && guhua.at<double>(x + 1, y + 1) > 0){
		image_pixel.at<double>(line_panduan, y + 1) = guhua.at<double>(x + 1, y + 1);
		return dfs2(guhua, x + 1, y + 1, w, l, line_panduan, image_pixel);
	}
}
...
void Cchair3DDlg::RenderScene()
{
	...
	//	}
	//}
	//glVertex3f(1, 1, 1); glVertex3f(1,0, 0); glVertex3f(0, 0, 1); glVertex3f(1, 0, 1);	//test
	glEnd();
	SwapBuffers(m_hDC);
}
...
//激光线骨架化
 Mat Cchair3DDlg::deal(Mat mat){
	...
	//Mat out(height,width, CV_32FC1, Scalar(0));//行数，列数----------------------修改
	Mat out(height, width, CV_64FC1, Scalar(0));
...
//	////	int w = 1292, l = 964;//col 竖列w=1292
	...
	Mat image = deal(imgRoi);								//---error
	Mat base_pixel(numofLine, width, CV_64FC1, Scalar(0));
	Mat image_pixel(numofLine, width, CV_64FC1, Scalar(0));					//采用最近邻搜索来避免匹配问题
	for (int j = 0; j < w;j++){
		int line = 0;
		for (int i = 0; i < l;i++){
	...
	//三维数据
	for (int i = 0; i < numofLine;i++){
		for (int j = 0; j < w;j++){					 //线条存在断裂的情况  
			double mm = image_pixel.at<double>(i, j);
			if (mm>0){
				double pixel = abs(image_pixel.at<double>(i,j) - base_pixel.at<double>(i,j));                     //像素差
				double h = len_base * pixel * size_pixel* 1.0 / ( F * 0.6 - pixel * size_pixel * cos(angle));      //sin(angle)=0.6直射式激光三角法计算像素偏移量对应的深度值                                            //直射式激光三角法  修改
				int x = base_pixel.at<double>(i,j);
